...
export async function POST(req: NextRequest) {
  ...
  try {
    if(action==="save_settings") {
      ...
      if(!sent||!items.length||items.some((x:any)=>!x.colour||!x.size||x.colour==="Custom"||x.size==="Custom"||x.requested_quantity===null))return NextResponse.json({error:"Choose a colour and size, or enter a complete custom value, for every requested variant."},{status:400});
