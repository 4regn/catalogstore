...
import { getReviewsPageAnalytics } from "../../../../lib/store-analytics";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analytics = await getReviewsPageAnalytics(admin, userData.user.id, range);
    return NextResponse.json({ ok: true, ...analytics });
  } catch (e: any) {
    console.error("Reviews page analytics fetch error:", e);
