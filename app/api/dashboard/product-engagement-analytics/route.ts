...
import { getProductEngagementAnalytics } from "../../../../lib/store-analytics";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analytics = await getProductEngagementAnalytics(admin, userData.user.id, range);
    return NextResponse.json({ ok: true, ...analytics });
  } catch (e: any) {
    console.error("Product engagement analytics fetch error:", e);
