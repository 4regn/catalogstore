...
import { getSearchInsightsAnalytics } from "../../../../lib/store-analytics";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analytics = await getSearchInsightsAnalytics(admin, userData.user.id, range);
    return NextResponse.json({ ok: true, ...analytics });
  } catch (e: any) {
    console.error("Search insights analytics fetch error:", e);
