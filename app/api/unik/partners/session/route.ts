...
const COOKIE = "unik-partner-access";

async function isPartner(authUserId: string) {
  const seller = await getUnikSeller();
  if (!seller) return false;
  const { data } = await getAdmin().from("unik_partners").select("id").eq("seller_id", seller.id).eq("auth_user_id", authUserId).maybeSingle();
  return !!data;
}
...
export async function GET(req: NextRequest) {
  ...
  if (error || !data.user || !(await isPartner(data.user.id))) {
...
export async function POST(req: NextRequest) {
  ...
  if (!(await isPartner(data.user.id))) return NextResponse.json({ error: "This account doesn't have Partner access" }, { status: 403 });
