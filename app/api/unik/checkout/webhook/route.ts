...
/* Yoco payment.succeeded webhook. Verifies the Svix-style signature, looks
   up the order, and idempotently marks it paid via markUnikOrderPaid --
   this is the primary confirmation path, but /api/unik/orders/[id] also
   self-heals by checking Yoco directly in case this never arrives or is
   delayed (webhook delivery/registration can be flaky, e.g. if signature
   verification or the endpoint URL is misconfigured). */
