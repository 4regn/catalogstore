...
/* Proxies a brand manager's own design image through our own origin, same
   reasoning as the partner Studio download route: a plain <a href> to a
   Supabase Storage URL is cross-origin so the `download` attribute is
   unreliable, and the clean original never needs to be public -- this signs
   it server-side on each request instead of exposing private_artwork_path
   directly. */
