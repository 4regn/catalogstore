...
export default function CrownStore({ initialSeller, initialProducts, initialDiscountCodes, initialProductId, isSubdomain }: StorePageProps = {}) {
  ...
  const EditSection = ({
    ...
    // A `style` prop here can be layout-critical (e.g. the hero's
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'Didact Gothic', sans-serif", background: displayBgColor, color: cream, minHeight: "100vh", overflowX: "hidden" }}>
        ...
        <StorefrontNavigationProgress active={navigating} color={gold} />
              ...
              {filtered.map((p, i) => {
                ...
                return (
                  <div key={p.id} className="crown-prod-card"
                    onClick={() => { if (isEditMode) openProduct(p); else { setNavigating(true); router.push(sp(p.handle ? `/products/${p.handle}` : `/p/${p.id}`)); } }}
                  ...
                  return (
                    <div key={i} onClick={() => { setActiveCategory(cat); document.getElementById("products")?.scrollIntoView({ behavior: "smooth" }); }}
                      ...
                      <div style={{ width: "100%", aspectRatio: "3/4", borderRadius: "200px 200px 12px 12px", overflow: "hidden", border: `1px solid ${border}`, background: bgCard, transition: "border-color 0.4s" }}>
                        {catImg ? (
                          <img src={catImg} alt={cat} onError={hideOnError} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", filter: "brightness(0.85)", transition: "transform 0.8s ease" }} />
                        ) : (
                ...
                {[
                  ...
                  return (
                    <div key={i} style={{ overflow: "hidden", borderRadius: cell.radius, position: "relative", background: bgCard }}>
                      {imgSrc
                        ? <img src={imgSrc} alt="" onError={hideOnError} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", filter: "brightness(0.85)" }} />
                ...
                <div style={{ paddingBottom: 32, maxHeight: "60vh", overflowY: "auto" }}>
                  {searched.length === 0 ? (
                    ...
                    searched.slice(0, 8).map((p) => (
                      <div key={p.id} onClick={() => { setShowSearch(false); setSearchQuery(""); if (isEditMode) openProduct(p); else { setNavigating(true); router.push(sp(p.handle ? `/products/${p.handle}` : `/p/${p.id}`)); } }}
                    ...
                    <div style={{ position: "absolute", bottom: 16, left: 16, display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {imgs.map((img, i) => (
                        <div key={i} onClick={e => { e.stopPropagation(); setActiveImg(i); }}
                          ...
                          <img src={img} alt="" onError={hideOnError} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top" }} />
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  return (
                    <div key={idx} style={{ display: "grid", gridTemplateColumns: "64px 1fr auto", gap: 14, alignItems: "center", padding: "16px 0", borderBottom: `1px solid ${border}` }}>
                      <div style={{ width: 64, height: 80, overflow: "hidden", background: bgCard, flexShrink: 0 }}>
                        {img ? <img src={img} alt="" onError={hideOnError} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: textMuted }}>◆</div>}
                      </div>
                      <div>
