    ...
    {loading ? <div className="loading">Loading your account…</div> : !data ? <section className="auth">
          ...
          return (
            <div className="laybuyplan" key={plan.id}>
              <div className="laybuyplan-top">
                ...
                <span className={`status ${plan.status === "paid_off" ? "delivered" : plan.status === "expired" ? "expired" : ""}`}>{plan.status === "paid_off" ? "Paid off" : plan.status === "expired" ? "Expired" : "Active"}</span>
              </div>
              ...
              {plan.status === "expired" && (
                <div className="laybuydeadline expired">This plan passed its 6-month payment window and is no longer active. Contact us if you&rsquo;d like to place a new order.</div>
              )}
              {plan.status === "active" && (
      ...
      {data.laybuyPlans.length === 0 && <section className="laybuypromo">
        <div className="laybuypromo-inner">
          ...
          <p>Pay a 30% deposit at checkout, clear the rest right here in your account &mdash; any amount, any time, within 6 months. No interest, no credit check.</p>
          <div className="laybuypromo-stats">
            ...
            <div><strong>6mo</strong><span>Time to pay off</span></div>
          </div>
