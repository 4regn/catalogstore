...
const formatDateTime = (value?: string | null) => {
  ...
  return date.toLocaleString("en-ZA", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
};
...
export default function FourRegnTrackingClient({ seller }: { seller: Seller }) {
  ...
  const trackingStages = tracking?.stages || FOUR_REGN_TRACKING_STAGES.map((stage) => ({ ...stage, complete: stage.key === "confirmed", current: stage.key === "confirmed" }));
  const trackingUpdatedAt = order ? formatDateTime(tracking?.updatedAt || order.tracking_updated_at || order.created_at) : "";
  return <main className="ft">
    ...
    {order && <section className="result tracking"><div className="resulthead"><div><span>ORDER FOUND</span><h2>{order.reference}</h2><p>{new Date(order.created_at).toLocaleDateString("en-ZA", { day:"numeric", month:"long", year:"numeric" })}</p>{trackingUpdatedAt && <p className="latest">Latest update: <strong>{trackingUpdatedAt}</strong></p>}</div><b>R {Number(order.total).toLocaleString("en-ZA")}</b></div>
      {tracking?.cancelled ? <p className="cancelled">This order was cancelled.</p> : <ol>{trackingStages.map((stage:any)=><li className={stage.complete?"done":""} key={stage.key}><i/><span>{stage.label || label(stage.key)}</span></li>)}</ol>}
      {tracking?.customerNote && <p className="customer-note"><strong>Update from 4REGN</strong>{tracking.customerNote}</p>}
