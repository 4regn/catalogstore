...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = `Customer Reviews & Testimonials | ${seller.store_name}`;
  const description = `Real reviews from real ${seller.store_name} customers — parcels arriving and outfits going on, straight from WhatsApp. Celebrating 7 years of ${seller.store_name}.`;

  return {
    title: sellerMetadataTitle(title, seller.custom_domain_status),
    description,
    alternates: { canonical: canonicalStoreUrl(slug, "/testimonials") },
    openGraph: { title, description, images: FOUR_REGN_TESTIMONIALS.slice(0, 4).map((t) => t.src) },
  };
}
...
export default async function TestimonialsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: seller } = await supabaseAdmin
    .from("sellers")
    .select("subdomain, store_name, logo_url, template, whatsapp_number, custom_domain_status")
    .eq("subdomain", slug)
    .maybeSingle<Seller>();
  ...
  return (
    <main className="testi-page">
      ...
      <section className="hero">
        <div className="eyebrow">Celebrating 7 years of 4REGN</div>
        ...
        <p>
          ...
          moment their parcel arrives — no filters, no staging, just people getting their order
          and telling us about it. Seven years in, this is what we&rsquo;re most proud of.
        </p>
      </section>

      <section className="grid">
        {FOUR_REGN_TESTIMONIALS.map((t, i) => (
          <figure className="card" key={t.src}>
            <img src={t.src} alt={`${seller.store_name} customer's WhatsApp message and photo after receiving their order`} width={t.width} height={t.height} loading={i < 2 ? "eager" : "lazy"} />
            <figcaption>
