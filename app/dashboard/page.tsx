...
export default function Dashboard() {
  ...
  const startEdit = (p: Product) => { const legacy = p.size_chart_html ? null : extractLegacyImportedSizeChart(p.description); const related = Array.isArray(p.metafields?.cart_booster_product_ids) ? p.metafields.cart_booster_product_ids.map(String) : []; const productCollections = storeCollections.filter((collection) => (p.category || "").split(",").some((value) => value.trim().toLowerCase() === collection.toLowerCase())); setEditingId(p.id); setFormName(p.name); setFormPrice(String(p.price)); setFormComparePrice(p.old_price ? String(p.old_price) : ""); setFormCategory(p.category || ""); setFormCollections(productCollections); setFormTags(p.tags || []); setFormCartBoosterIds(related); setCartBoosterSearch(""); setFormDescription(legacy?.description ?? p.description ?? ""); setFormSizeChartHtml(p.size_chart_html || legacy?.sizeChartHtml || ""); setFormSourceUrl(p.source_url || ""); setFormInStock(p.in_stock); setFormImages([]); setFormPreviews([]); setExistingImages(p.images || []); setFormVariants(p.variants || []); setFormImportAsDraft(false); setShowForm(true);
    // Clicking Edit further down a long product list otherwise leaves the
    // form open off-screen above the click point -- same scrollIntoView
    // already used for the ?editProductHandle= deep-link flow below, just
    // also fired here so every path into the edit form lands you on it.
    setTimeout(() => { document.getElementById("product-edit-form")?.scrollIntoView({ behavior: "smooth", block: "start" }); }, 50);
  };
            ...
            {selectedOrder ? (
              <div>
                <div style={{ padding: "24px 20px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 16, marginBottom: 16 }}>
                  ...
                  {seller?.subdomain === "4regn" && (() => {
                    // Passing trackingHistory means every completed stage
                    // gets its own real timestamp (see buildFourRegnTracking's
                    // own comment) instead of only ever the single most
                    // recent status change -- exactly the same fix applied
                    // to the customer-facing tracking page itself.
                    const tracking = buildFourRegnTracking(selectedOrder, trackingHistory);
                    return <div style={{ padding: 16, borderRadius: 14, background: "var(--panel-2)", border: "1px solid var(--border)" }}>
                      ...
                      {tracking.cancelled ? <div style={{ color: "#ff6b35", fontSize: 11, fontWeight: 800 }}>Order cancelled</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(105px,1fr))", gap: 7 }}>{tracking.stages.map((stage) => {
                        const timestamp = stage.occurredAt;
