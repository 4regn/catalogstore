      ...
      <div style={{ minHeight: "100vh", background: "#030303", display: "flex", fontFamily: "'Schibsted Grotesk', sans-serif", color: "#f5f5f5" }}>
        ...
        <aside className={"sidebar" + (sidebarOpen ? " open" : "")} style={{ width: 260, borderRight: "1px solid rgba(255,255,255,0.05)", padding: "24px 20px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "fixed", top: 0, left: 0, bottom: 0, background: "#080808", zIndex: 100, transition: "transform 0.3s ease" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
            <div style={{ fontSize: 18, fontWeight: 900, letterSpacing: "-0.04em", textTransform: "uppercase" as const }}>CATALOG<span style={{ background: G, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>STORE</span></div>
        ...
        <main className="main-content" style={{ flex: 1, marginLeft: 260, padding: "36px 40px" }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-grid-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <input type="number" placeholder="Original price e.g. 399 (shows crossed out)" value={formComparePrice} onChange={(e) => setFormComparePrice(e.target.value)} style={{ width: "100%", padding: "10px 14px", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.06)", borderRadius: 10, color: "rgba(245,245,245,0.5)", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} />
                    {formComparePrice && parseFloat(formComparePrice) > parseFloat(formPrice || "0") && (
                      <span style={{ fontSize: 10, color: "#22c55e", fontWeight: 700 }}>{Math.round((1 - parseFloat(formPrice) / parseFloat(formComparePrice)) * 100)}% off — <span style={{ textDecoration: "line-through", color: "rgba(245,245,245,0.3)" }}>R{formComparePrice}</span> → R{formPrice}</span>
                    )}
                  </div>
                </div>
                            ...
                            return (
                              <div key={oi} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                <div style={{ display: "flex", gap: 4, flex: 1, overflowX: "auto" as const }}>
                                  ...
                                  {allImgs.map((img, imgIdx) => (<img key={imgIdx} src={img} alt="" onClick={() => { const u = [...formVariants]; if (!u[vi].images) u[vi].images = {}; u[vi].images![opt] = img; setFormVariants(u); }} style={{ width: 36, height: 36, borderRadius: 6, objectFit: "cover" as const, cursor: "pointer", border: currentImg === img ? "2px solid " + N : "1px solid rgba(255,255,255,0.08)", flexShrink: 0, opacity: currentImg === img ? 1 : 0.5 }} />))}
                                </div>
                              </div>
                            );
                          })}
                ...
                {filteredProducts.map((product) => (
                  <div key={product.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: 12, flexWrap: "wrap" as const, gap: 12, opacity: product.status === "trashed" ? 0.6 : 1 }} className="product-row-inner">
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                      <div style={{ fontSize: 16, fontWeight: 900, letterSpacing: "-0.03em", whiteSpace: "nowrap" as const, color: product.old_price ? N : "#f5f5f5" }}>R{product.price}</div>
          ...
          {tab === "collections" && (<div>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8, flexWrap: "wrap" as const, gap: 12 }}>
              <div><h1 style={{ fontSize: "clamp(20px, 4vw, 28px)", fontWeight: 900, letterSpacing: "-0.04em", textTransform: "uppercase" as const, marginBottom: 4 }}>Collections</h1><p style={{ fontSize: 14, color: "rgba(245,245,245,0.35)", marginBottom: 24 }}>Organize your products into collections for your storefront.</p></div>
            ...
            {selectedCollection ? (
              <div>
                ...
                {(() => {
                  ...
                  return inCollection.length === 0 ? <p style={{ fontSize: 13, color: "rgba(245,245,245,0.2)", padding: "20px 0" }}>No products in this collection yet.</p> : (
                      ...
                      return (
                        <div key={i} onClick={() => setSelectedCollection(col)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: 12, cursor: "pointer", transition: "border-color 0.2s" }} onMouseEnter={(e) => e.currentTarget.style.borderColor = "rgba(255,107,53,0.15)"} onMouseLeave={(e) => e.currentTarget.style.borderColor = "rgba(255,255,255,0.04)"}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            <div><div style={{ fontSize: 14, fontWeight: 700, textTransform: "uppercase" as const, marginBottom: 2 }}>{col}</div><div style={{ fontSize: 11, color: "rgba(245,245,245,0.25)" }}>{count} product{count !== 1 ? "s" : ""}</div></div>
                          </div>
          ...
          {tab === "orders" && (<div>
            ...
            {!selectedOrder && visibleOrders.length > 0 && <p style={{ fontSize: 12, color: "rgba(245,245,245,0.2)", marginBottom: 16 }}>{visibleOrders.length} order{visibleOrders.length !== 1 ? "s" : ""}</p>}
            {selectedOrder ? (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ padding: "20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16 }}>
                    ...
                    {selectedOrder.fulfillment_method === "pickup" ? <div style={{ fontSize: 13, color: "rgba(245,245,245,0.35)" }}>Customer will pick up</div> : selectedOrder.shipping_address ? (
                      <div style={{ fontSize: 13, color: "rgba(245,245,245,0.35)", lineHeight: 1.6 }}>{selectedOrder.shipping_address.address}{selectedOrder.shipping_address.apartment ? ", " + selectedOrder.shipping_address.apartment : ""}<br />{selectedOrder.shipping_address.city}, {selectedOrder.shipping_address.province}<br />{selectedOrder.shipping_address.postal_code}</div>
                ...
                <div style={{ padding: "20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16 }}>
                  ...
                  {(selectedOrder.items || []).map((item, i) => {
                    ...
                    return (<div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i < selectedOrder.items.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                      ...
                      <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600 }}>{item.name}</div>{item.variant && <div style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginTop: 2 }}>{item.variant}</div>}<div style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginTop: 2 }}>Qty: {item.qty}</div></div>
          ...
          {tab === "abandoned" && (<div>
            ...
            {abandonedOrders.length === 0 ? (
              <div style={{ textAlign: "center" as const, padding: "60px 20px", color: "rgba(245,245,245,0.35)" }}><p style={{ fontSize: 16, fontWeight: 800, textTransform: "uppercase" as const, marginBottom: 8 }}>No abandoned checkouts</p><p style={{ fontSize: 13, color: "rgba(245,245,245,0.2)" }}>When customers leave without paying, they'll show up here.</p></div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {abandonedOrders.map((order) => (
                  <div key={order.id} style={{ padding: "16px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" as const, gap: 12, marginBottom: 10 }}>
                      <div><div style={{ fontSize: 13, fontWeight: 700, textTransform: "uppercase" as const }}>#{order.order_number} - {order.customer_name || "Unknown"}</div><div style={{ fontSize: 10, color: "rgba(245,245,245,0.25)", marginTop: 2 }}>{order.customer_email} {order.customer_phone ? " / " + order.customer_phone : ""}</div></div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 16, fontWeight: 900 }}>R{order.total}</span><span style={{ padding: "5px 10px", borderRadius: 100, fontSize: 9, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", background: "rgba(255,61,110,0.08)", color: "#ff3d6e" }}>Abandoned</span></div>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const, fontSize: 11, color: "rgba(245,245,245,0.2)" }}>
                      {(order.items || []).map((item, i) => (<span key={i} style={{ padding: "4px 10px", background: "rgba(255,255,255,0.03)", borderRadius: 6, border: "1px solid rgba(255,255,255,0.04)" }}>{item.name} x{item.qty}</span>))}
          ...
          {tab === "discounts" && (<div>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap" as const, gap: 12 }}>
              <div><h1 style={{ fontSize: "clamp(20px, 4vw, 28px)", fontWeight: 900, letterSpacing: "-0.04em", textTransform: "uppercase" as const, marginBottom: 4 }}>Discount Codes</h1><p style={{ fontSize: 14, color: "rgba(245,245,245,0.35)" }}>Create promo codes for cart, products, collections, or shipping.</p></div>
            ...
            {showDcForm && (
              <div style={{ padding: "28px 24px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, marginBottom: 24 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Discount Code</label><input type="text" value={dcCode} onChange={(e) => setDcCode(e.target.value.toUpperCase().replace(/\s/g, ""))} placeholder="e.g. WELCOME10" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 14, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", fontWeight: 700, letterSpacing: "0.04em" }} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Applies To</label>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Discount Amount</label>
                    <div style={{ display: "flex", gap: 0, border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, overflow: "hidden" }}>
                      <div style={{ display: "flex" }}>{[{ k: "percentage", l: "%" }, { k: "fixed", l: "R" }].map((t) => (<button key={t.k} onClick={() => { setDcType(t.k); if (t.k === "percentage" && parseFloat(dcValue) > 100) setDcValue("100"); }} style={{ padding: "12px 16px", background: dcType === t.k ? "rgba(255,107,53,0.12)" : "rgba(255,255,255,0.03)", border: "none", borderRight: "1px solid rgba(255,255,255,0.06)", color: dcType === t.k ? N : "rgba(245,245,245,0.35)", fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: 14, fontWeight: 800, cursor: "pointer" }}>{t.l}</button>))}</div>
                  ...
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Preview</label><div style={{ padding: "12px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, fontSize: 16, fontWeight: 800, color: dcValue ? N : "rgba(245,245,245,0.15)" }}>{dcValue ? (dcType === "percentage" ? dcValue + "% OFF" : "R" + dcValue + " OFF") : "Set amount"}</div></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Min Order (R)</label><input type="text" inputMode="numeric" value={dcMinOrder} onChange={(e) => setDcMinOrder(e.target.value.replace(/[^0-9]/g, ""))} placeholder="0" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Max Uses</label><input type="text" inputMode="numeric" value={dcMaxUses} onChange={(e) => setDcMaxUses(e.target.value.replace(/[^0-9]/g, ""))} placeholder="Unlimited" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Expires</label><input type="date" value={dcExpires} onChange={(e) => setDcExpires(e.target.value)} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                </div>
                {dcAppliesTo === "product" && (<div style={{ marginBottom: 16 }}><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 8, display: "block" }}>Select Products</label><div style={{ maxHeight: 200, overflow: "auto", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, background: "rgba(255,255,255,0.02)" }}>{products.filter((p) => (p.status || "published") !== "trashed").map((p) => (<div key={p.id} onClick={() => setDcProductIds(dcProductIds.includes(p.id) ? dcProductIds.filter((x) => x !== p.id) : [...dcProductIds, p.id])} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid rgba(255,255,255,0.03)", background: dcProductIds.includes(p.id) ? "rgba(255,107,53,0.04)" : "transparent" }}><div style={{ width: 18, height: 18, borderRadius: 4, border: dcProductIds.includes(p.id) ? "2px solid " + N : "1px solid rgba(255,255,255,0.12)", background: dcProductIds.includes(p.id) ? "rgba(255,107,53,0.15)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: N }}>{dcProductIds.includes(p.id) ? "\u2713" : ""}</div>{p.image_url ? <img src={p.image_url} alt="" style={{ width: 28, height: 28, borderRadius: 4, objectFit: "cover" }} /> : <div style={{ width: 28, height: 28, borderRadius: 4, background: "rgba(255,255,255,0.04)" }} />}<span style={{ fontSize: 13, fontWeight: 500 }}>{p.name}</span><span style={{ fontSize: 11, color: "rgba(245,245,245,0.25)", marginLeft: "auto" }}>R{p.price}</span></div>))}</div>{dcProductIds.length > 0 && <p style={{ fontSize: 11, color: N, marginTop: 6 }}>{dcProductIds.length} product{dcProductIds.length !== 1 ? "s" : ""} selected</p>}</div>)}
                {dcAppliesTo === "collection" && (<div style={{ marginBottom: 16 }}><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 8, display: "block" }}>Select Collections</label><div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>{storeCollections.map((col) => (<button key={col} onClick={() => setDcCollections(dcCollections.includes(col) ? dcCollections.filter((x) => x !== col) : [...dcCollections, col])} style={{ padding: "8px 16px", borderRadius: 100, background: dcCollections.includes(col) ? "rgba(255,107,53,0.08)" : "rgba(255,255,255,0.03)", border: dcCollections.includes(col) ? "1px solid rgba(255,107,53,0.15)" : "1px solid rgba(255,255,255,0.06)", color: dcCollections.includes(col) ? N : "rgba(245,245,245,0.35)", fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: 11, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" as const }}>{col}</button>))}</div>{dcCollections.length > 0 && <p style={{ fontSize: 11, color: N, marginTop: 6 }}>{dcCollections.length} collection{dcCollections.length !== 1 ? "s" : ""} selected</p>}</div>)}
                {dcAppliesTo === "shipping" && (<div style={{ padding: "12px 16px", background: "rgba(251,191,36,0.04)", border: "1px solid rgba(251,191,36,0.1)", borderRadius: 10, marginBottom: 16, fontSize: 12, color: "#fbbf24" }}>Shipping discounts apply to delivery fees only.</div>)}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", marginBottom: 16 }}>
                  <div><span style={{ fontSize: 13 }}>Show countdown timer on store</span><br /><span style={{ fontSize: 11, color: "rgba(245,245,245,0.25)" }}>Display a countdown to expiry on product pages</span></div>
            ...
            {!showDcForm && discountCodes.length === 0 ? (
              <div style={{ textAlign: "center" as const, padding: "60px 20px", color: "rgba(245,245,245,0.35)" }}><p style={{ fontSize: 16, fontWeight: 800, textTransform: "uppercase" as const, marginBottom: 8 }}>No discount codes yet</p><p style={{ fontSize: 13, color: "rgba(245,245,245,0.2)" }}>Create your first code to start offering promotions.</p></div>
            ) : !showDcForm && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  return (
                    <div key={cat.key}>
                      <button onClick={() => setOpenDiscountCat(openDiscountCat === cat.key ? null : cat.key)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: openDiscountCat === cat.key ? "12px 12px 0 0" : 12, cursor: "pointer", fontFamily: "'Schibsted Grotesk', sans-serif" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}><h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.04em", margin: 0, color: "#f5f5f5" }}>{cat.label}</h3><span style={{ fontSize: 11, color: "rgba(245,245,245,0.25)" }}>({catCodes.length})</span></div>
                      ...
                      {openDiscountCat === cat.key && (
                        <div style={{ border: "1px solid rgba(255,255,255,0.05)", borderTop: "none", borderRadius: "0 0 12px 12px", background: "rgba(255,255,255,0.01)" }}>
                          {catCodes.length === 0 ? <p style={{ padding: "20px 18px", fontSize: 12, color: "rgba(245,245,245,0.2)" }}>No codes yet.</p> : catCodes.map((dc) => (
                            <div key={dc.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid rgba(255,255,255,0.03)", flexWrap: "wrap" as const, gap: 10 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                ...
                                <div><div style={{ fontSize: 13, fontWeight: 700 }}>{dc.type === "percentage" ? dc.value + "% off" : "R" + dc.value + " off"}</div><div style={{ fontSize: 10, color: "rgba(245,245,245,0.2)", marginTop: 2 }}>{dc.min_order > 0 ? "Min R" + dc.min_order + " " : ""}{dc.max_uses ? "Max " + dc.max_uses + " uses " : "Unlimited "}- Used {dc.used_count}x{dc.expires_at ? " - Exp " + new Date(dc.expires_at).toLocaleDateString() : ""}</div></div>
                              </div>
          ...
          {tab === "mystore" && (<div>
            ...
            {storeTemplate !== "heirloom" && (<div style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                {([{ key: "show_banner_text" as const, label: "Banner Text Overlay", desc: "Show title and subtitle on your banner image" }, { key: "show_marquee" as const, label: "Announcement Ticker", desc: "Scrolling marquee below header" }, { key: "show_collections" as const, label: "Collections Section", desc: "Display collection cards on homepage" }, { key: "show_about" as const, label: "About / Brand Story", desc: "Your brand story with image" }, { key: "show_trust_bar" as const, label: "Trust Bar", desc: "Shipping, returns, quality, payment icons" }, { key: "show_policies" as const, label: "Shipping & Policies", desc: "Detailed shipping, returns, and payment info" }]).map((item) => (
                  <div key={item.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12 }}>
                    <div><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{item.label}</div><div style={{ fontSize: 11, color: "rgba(245,245,245,0.2)" }}>{item.desc}</div></div>
                    <button onClick={() => setStoreConfig({ ...storeConfig, [item.key]: !storeConfig[item.key] })} style={{ width: 48, height: 28, borderRadius: 100, border: "none", cursor: "pointer", position: "relative" as const, background: storeConfig[item.key] ? N : "rgba(255,255,255,0.08)", transition: "background 0.2s" }}><div style={{ width: 22, height: 22, borderRadius: "50%", background: "#fff", position: "absolute" as const, top: 3, left: storeConfig[item.key] ? 23 : 3, transition: "left 0.2s", boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }} /></button>
                  </div>
                ))}
              </div>
            </div>)}
            <div style={{ marginBottom: 40 }}><h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Announcement Bar</h3><p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 12 }}>Shows at the very top of your store. Leave empty to hide.</p><input type="text" placeholder="e.g. Free delivery on orders over R500" value={storeConfig.announcement} onChange={(e) => setStoreConfig({ ...storeConfig, announcement: e.target.value })} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
            {(storeTemplate === "heirloom" || storeConfig.show_marquee) && (<div style={{ marginBottom: 40 }}><h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Ticker Messages</h3><p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 12 }}>Scrolling text shown below the header.</p>{storeConfig.marquee_texts.map((txt, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}><input type="text" value={txt} onChange={(e) => { const u = [...storeConfig.marquee_texts]; u[i] = e.target.value; setStoreConfig({ ...storeConfig, marquee_texts: u }); }} style={{ flex: 1, padding: "10px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} />{storeConfig.marquee_texts.length > 1 && <button onClick={() => { const u = storeConfig.marquee_texts.filter((_, idx) => idx !== i); setStoreConfig({ ...storeConfig, marquee_texts: u }); }} style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(255,61,110,0.06)", border: "1px solid rgba(255,61,110,0.12)", color: "#ff3d6e", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>&times;</button>}</div>))}<button onClick={() => setStoreConfig({ ...storeConfig, marquee_texts: [...storeConfig.marquee_texts, ""] })} style={{ padding: "8px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 100, color: "rgba(245,245,245,0.35)", fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: 11, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" as const, marginTop: 4 }}>+ Add Message</button></div>)}
            {storeTemplate !== "heirloom" && storeConfig.show_trust_bar && (<div style={{ marginBottom: 40 }}><h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Trust Bar</h3><p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 12 }}>Select an icon and add a title/description for each item.</p>{storeConfig.trust_items.map((item, i) => (<div key={i} style={{ padding: "12px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, marginBottom: 8 }}><div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const, marginBottom: 8 }}>{[
            ...
            ].map(({ id, svg }) => (<button key={id} title={id} onClick={() => { const u = [...storeConfig.trust_items]; u[i] = { ...u[i], icon: id }; setStoreConfig({ ...storeConfig, trust_items: u }); }} style={{ width: 32, height: 32, borderRadius: 6, border: item.icon === id ? `2px solid ${N}` : "1px solid rgba(255,255,255,0.08)", background: item.icon === id ? `${N}15` : "rgba(255,255,255,0.03)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: item.icon === id ? N : "rgba(245,245,245,0.5)" }}>{svg}</button>))}</div><div style={{ display: "flex", gap: 8 }}><input type="text" value={item.title} onChange={(e) => { const u = [...storeConfig.trust_items]; u[i] = { ...u[i], title: e.target.value }; setStoreConfig({ ...storeConfig, trust_items: u }); }} placeholder="Title" style={{ flex: 1, padding: "8px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /><input type="text" value={item.desc} onChange={(e) => { const u = [...storeConfig.trust_items]; u[i] = { ...u[i], desc: e.target.value }; setStoreConfig({ ...storeConfig, trust_items: u }); }} placeholder="Description" style={{ flex: 2, padding: "8px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} />{storeConfig.trust_items.length > 1 && <button onClick={() => { const u = storeConfig.trust_items.filter((_, idx) => idx !== i); setStoreConfig({ ...storeConfig, trust_items: u }); }} style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(255,61,110,0.06)", border: "none", color: "#ff3d6e", fontSize: 12, cursor: "pointer", flexShrink: 0 }}>&times;</button>}</div></div>))}{storeConfig.trust_items.length < 6 && <button onClick={() => setStoreConfig({ ...storeConfig, trust_items: [...storeConfig.trust_items, { icon: "shield", title: "", desc: "" }] })} style={{ padding: "8px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 100, color: "rgba(245,245,245,0.35)", fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: 11, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" as const, marginTop: 4 }}>+ Add Item</button>}</div>)}
            {storeTemplate !== "heirloom" && storeConfig.show_policies && (<div style={{ marginBottom: 40 }}><h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Shipping & Policies</h3><p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 12 }}>Edit your shipping, returns, and payment policy text.</p>{storeConfig.policy_items.map((item, i) => (<div key={i} style={{ padding: "12px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, marginBottom: 8 }}><input type="text" value={item.title} onChange={(e) => { const u = [...storeConfig.policy_items]; u[i] = { ...u[i], title: e.target.value }; setStoreConfig({ ...storeConfig, policy_items: u }); }} placeholder="e.g. Shipping" style={{ width: "100%", padding: "8px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#f5f5f5", fontSize: 12, fontWeight: 700, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", marginBottom: 6, textTransform: "uppercase" as const, letterSpacing: "0.04em" }} /><textarea value={item.desc} onChange={(e) => { const u = [...storeConfig.policy_items]; u[i] = { ...u[i], desc: e.target.value }; setStoreConfig({ ...storeConfig, policy_items: u }); }} placeholder="Policy details..." rows={2} style={{ width: "100%", padding: "8px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div>))}</div>)}
            <div style={{ marginBottom: 40 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}>
                {([{ key: "instagram" as const, label: "Instagram", placeholder: "https://instagram.com/yourbrand" }, { key: "tiktok" as const, label: "TikTok", placeholder: "https://tiktok.com/@yourbrand" }, { key: "facebook" as const, label: "Facebook", placeholder: "https://facebook.com/yourbrand" }, { key: "twitter" as const, label: "X / Twitter", placeholder: "https://x.com/yourbrand" }]).map((item) => (
                  <div key={item.key} style={{ display: "flex", alignItems: "center", gap: 12 }}><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.08em", textTransform: "uppercase" as const, width: 80, flexShrink: 0 }}>{item.label}</label><input type="url" placeholder={item.placeholder} value={socialLinks[item.key] || ""} onChange={(e) => setSocialLinks({ ...socialLinks, [item.key]: e.target.value })} style={{ flex: 1, padding: "11px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: 40 }}>
              <h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Contact & Store Info</h3>
              <p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 16 }}>Shown in the footer contact popup. Leave empty to hide.</p>
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.08em", textTransform: "uppercase" as const, width: 80, flexShrink: 0 }}>Email</label><input type="email" placeholder="orders@yourbrand.co.za" value={storeConfig.contact_email || ""} onChange={(e) => setStoreConfig({ ...storeConfig, contact_email: e.target.value })} style={{ flex: 1, padding: "11px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.08em", textTransform: "uppercase" as const, width: 80, flexShrink: 0 }}>Phone</label><input type="tel" placeholder="+27 12 345 6789" value={storeConfig.contact_phone || ""} onChange={(e) => setStoreConfig({ ...storeConfig, contact_phone: e.target.value })} style={{ flex: 1, padding: "11px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div>
                <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Operating Hours</label><textarea value={storeConfig.operating_hours || ""} onChange={(e) => setStoreConfig({ ...storeConfig, operating_hours: e.target.value })} placeholder="e.g. Mon-Fri 9am-5pm, Sat 10am-2pm" rows={2} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div>
                <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Physical Address</label><textarea value={storeConfig.physical_address || ""} onChange={(e) => setStoreConfig({ ...storeConfig, physical_address: e.target.value })} placeholder="e.g. 123 Main Street, Cape Town" rows={2} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div>
              </div>
            </div>
            <div style={{ marginBottom: 40 }}>
              <h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Policies</h3>
              <p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 16 }}>Displayed in the footer when customers click Shipping or Returns.</p>
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}>
                <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Shipping Policy</label><textarea value={storeConfig.shipping_policy || ""} onChange={(e) => setStoreConfig({ ...storeConfig, shipping_policy: e.target.value })} placeholder="e.g. We deliver nationwide within 3-5 business days..." rows={4} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div>
                <div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Return / Refund Policy</label><textarea value={storeConfig.return_policy || ""} onChange={(e) => setStoreConfig({ ...storeConfig, return_policy: e.target.value })} placeholder="e.g. We accept returns within 14 days of purchase..." rows={4} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div>
              </div>
            </div>
            <div style={{ marginBottom: 40 }}>
              <h3 style={{ fontSize: 13, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.06em", marginBottom: 8 }}>Free Shipping Threshold</h3>
              <p style={{ fontSize: 12, color: "rgba(245,245,245,0.25)", marginBottom: 12 }}>Orders above this amount qualify for free shipping. Leave empty to disable.</p>
              <input type="number" placeholder="e.g. 500" value={storeConfig.free_ship_threshold ?? ""} onChange={(e) => setStoreConfig({ ...storeConfig, free_ship_threshold: e.target.value ? Number(e.target.value) : null })} style={{ width: 160, padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} />
            </div>
          ...
          {tab === "checkout" && (<div>
            ...
            <div style={{ marginBottom: 40 }}>
              ...
              {checkoutConfig.delivery_enabled && (<>
                {checkoutConfig.shipping_options.map((opt, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center" }}><span style={{ flex: 1, padding: "10px 14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, fontSize: 13, color: "#f5f5f5" }}>{opt.name} - <span style={{ color: N }}>R{opt.price}</span></span><button onClick={() => setCheckoutConfig({ ...checkoutConfig, shipping_options: checkoutConfig.shipping_options.filter((_, idx) => idx !== i) })} style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(255,61,110,0.06)", border: "none", color: "#ff3d6e", fontSize: 14, cursor: "pointer" }}>&times;</button></div>))}
            ...
            <div style={{ marginBottom: 40 }}>
              ...
              {checkoutConfig.pickup_enabled && (<div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}><div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Pickup Address</label><input type="text" value={checkoutConfig.pickup_address} onChange={(e) => setCheckoutConfig({ ...checkoutConfig, pickup_address: e.target.value })} placeholder="e.g. 123 Main Street, Durban" style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} /></div><div><label style={{ fontSize: 11, fontWeight: 700, color: "rgba(245,245,245,0.35)", letterSpacing: "0.1em", textTransform: "uppercase" as const, marginBottom: 6, display: "block" }}>Pickup Instructions</label><textarea value={checkoutConfig.pickup_instructions} onChange={(e) => setCheckoutConfig({ ...checkoutConfig, pickup_instructions: e.target.value })} placeholder="e.g. Open Mon-Fri 9am-5pm." rows={3} style={{ width: "100%", padding: "12px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, color: "#f5f5f5", fontSize: 13, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none", resize: "vertical" as const }} /></div></div>)}
            </div>
            <div style={{ marginBottom: 40 }}>
