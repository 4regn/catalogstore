...
export default function Dashboard() {
  ...
  const navSections: { label: string | null; items: { key: TabKey; name: string; count?: number }[] }[] = [
    { label: null, items: [{ key: "overview", name: "Overview" }] },
    {
      label: "Sell",
      items: [
        { key: "products", name: "Products", count: publishedCount },
        { key: "collections", name: "Collections", count: storeCollections.length },
        { key: "orders", name: "Orders", count: visibleOrders.length },
        { key: "abandoned", name: "Abandoned", count: abandonedOrders.length },
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", fontFamily: "'Schibsted Grotesk', sans-serif", color: "var(--text)" }}>
        ...
        <aside className={"sidebar" + (sidebarOpen ? " open" : "")} style={{ width: 260, borderRight: "1px solid var(--border)", padding: "24px 16px", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: 24, position: "fixed", top: 0, left: 0, bottom: 0, background: "var(--panel-solid)", zIndex: 100, transition: "transform 0.3s ease", overflowY: "auto" as const }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ padding: "14px 16px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 12 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--muted-2)", textTransform: "uppercase" as const, letterSpacing: "0.08em", fontWeight: 600 }}>{seller?.plan || "Free"} plan</div>
            </div>
        ...
        <main className="main-content" style={{ flex: 1, marginLeft: 260, padding: "36px 40px" }}>
          ...
          {tab === "overview" && (<div>
            ...
            <div className="actions-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            ...
            {showDcForm && (
              <div style={{ padding: "28px 24px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 16, marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", marginBottom: 16 }}>
                  <div><span style={{ fontSize: 13 }}>Show countdown timer on store</span><br /><span style={{ fontSize: 11, color: "var(--muted-2)" }}>Display a countdown to expiry on product pages</span></div>
                          ...
                          {catCodes.length === 0 ? <p style={{ padding: "20px 18px", fontSize: 12, color: "var(--muted-2)" }}>No codes yet.</p> : catCodes.map((dc) => (
                            <div key={dc.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid var(--border)", flexWrap: "wrap" as const, gap: 10 }}>
                              ...
                              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                                <button onClick={() => { setDcEditId(dc.id); setDcCode(dc.code); setDcType(dc.type); setDcValue(String(dc.value)); setDcMinOrder(dc.min_order > 0 ? String(dc.min_order) : ""); setDcMaxUses(dc.max_uses ? String(dc.max_uses) : ""); setDcExpires(dc.expires_at ? dc.expires_at.split("T")[0] : ""); setDcAppliesTo(dc.applies_to || "cart"); setDcProductIds(dc.product_ids || []); setDcCollections(dc.collection_names || []); setDcShowCountdown(dc.show_countdown || false); setShowDcForm(true); }} style={{ padding: "5px 12px", borderRadius: 100, fontSize: 10, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.04em", cursor: "pointer", border: "1px solid var(--border)", background: "var(--panel-2)", color: "var(--muted)", fontFamily: "'Schibsted Grotesk', sans-serif" }}>Edit</button>
          ...
          {tab === "mystore" && (<div>
            ...
            {storeTemplate !== "heirloom" && (<div style={sectionCard}>
              <h3 style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "var(--muted)", marginBottom: 8 }}>Section Visibility</h3>
              <p style={{ fontSize: 12, color: "var(--muted-2)", marginBottom: 16 }}>Toggle which sections appear on your store.</p>
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                {([{ key: "show_banner_text" as const, label: "Banner Text Overlay", desc: "Show title and subtitle on your banner image" }, { key: "show_marquee" as const, label: "Announcement Ticker", desc: "Scrolling marquee below header" }, { key: "show_collections" as const, label: "Collections Section", desc: "Display collection cards on homepage" }, { key: "show_about" as const, label: "About / Brand Story", desc: "Your brand story with image" }, { key: "show_trust_bar" as const, label: "Trust Bar", desc: "Shipping, returns, quality, payment icons" }, { key: "show_policies" as const, label: "Shipping & Policies", desc: "Detailed shipping, returns, and payment info" }]).map((item) => (
                  <div key={item.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 12 }}>
                    ...
                    <button onClick={() => setStoreConfig({ ...storeConfig, [item.key]: !storeConfig[item.key] })} style={{ width: 48, height: 28, borderRadius: 100, border: "none", cursor: "pointer", position: "relative" as const, background: storeConfig[item.key] ? N : "var(--toggle-off)", transition: "background 0.2s" }}><div style={{ width: 22, height: 22, borderRadius: "50%", background: "#fff", position: "absolute" as const, top: 3, left: storeConfig[item.key] ? 23 : 3, transition: "left 0.2s", boxShadow: "0 1px 4px rgba(0,0,0,0.2)" }} /></button>
                  </div>
                ))}
              </div>
            </div>)}
            <div style={sectionCard}><h3 style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "var(--muted)", marginBottom: 8 }}>Announcement Bar</h3><p style={{ fontSize: 12, color: "var(--muted-2)", marginBottom: 12 }}>Shows at the very top of your store. Leave empty to hide.</p><input type="text" placeholder="e.g. Free delivery on orders over R500" value={storeConfig.announcement} onChange={(e) => setStoreConfig({ ...storeConfig, announcement: e.target.value })} style={inputStyle} /></div>
            {(storeTemplate === "heirloom" || storeConfig.show_marquee) && (<div style={sectionCard}><h3 style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "var(--muted)", marginBottom: 8 }}>Ticker Messages</h3><p style={{ fontSize: 12, color: "var(--muted-2)", marginBottom: 12 }}>Scrolling text shown below the header.</p>{storeConfig.marquee_texts.map((txt, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}><input type="text" value={txt} onChange={(e) => { const u = [...storeConfig.marquee_texts]; u[i] = e.target.value; setStoreConfig({ ...storeConfig, marquee_texts: u }); }} style={{ flex: 1, padding: "10px 14px", background: "var(--input-bg)", border: "1px solid var(--border)", borderRadius: 12, color: "var(--text)", fontSize: 12, fontFamily: "'Schibsted Grotesk', sans-serif", outline: "none" }} />{storeConfig.marquee_texts.length > 1 && <button onClick={() => { const u = storeConfig.marquee_texts.filter((_, idx) => idx !== i); setStoreConfig({ ...storeConfig, marquee_texts: u }); }} style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(255,61,110,0.06)", border: "1px solid rgba(255,61,110,0.12)", color: "#ff3d6e", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>&times;</button>}</div>))}<button onClick={() => setStoreConfig({ ...storeConfig, marquee_texts: [...storeConfig.marquee_texts, ""] })} style={{ padding: "8px 16px", background: "var(--panel-2)", border: "1px solid var(--border)", borderRadius: 100, color: "var(--muted)", fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: 11, fontWeight: 700, cursor: "pointer", textTransform: "uppercase" as const, marginTop: 4 }}>+ Add Message</button></div>)}
            {storeTemplate !== "heirloom" && storeConfig.show_trust_bar && (<div style={sectionCard}><h3 style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "var(--muted)", marginBottom: 8 }}>Trust Bar</h3><p style={{ fontSize: 12, color: "var(--muted-2)", marginBottom: 12 }}>Select an icon and add a title/description for each item.</p>{storeConfig.trust_items.map((item, i) => (<div key={i} style={{ padding: "12px 14px", background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 12, marginBottom: 8 }}><div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const, marginBottom: 8 }}>{[
            ...
            <div style={sectionCard}>
              <h3 style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase" as const, letterSpacing: "0.1em", color: "var(--muted)", marginBottom: 8 }}>Policies</h3>
              <p style={{ fontSize: 12, color: "var(--muted-2)", marginBottom: 16 }}>Displayed in the footer when customers click Shipping or Returns.</p>
              <div style={{ display: "flex", flexDirection: "column" as const, gap: 12 }}>
                <div><label style={labelStyle}>Shipping Policy</label><textarea value={storeConfig.shipping_policy || ""} onChange={(e) => setStoreConfig({ ...storeConfig, shipping_policy: e.target.value })} placeholder="e.g. We deliver nationwide within 3-5 business days..." rows={4} style={{ ...inputStyle, fontSize: 12, resize: "vertical" as const }} /></div>
                <div><label style={labelStyle}>Return / Refund Policy</label><textarea value={storeConfig.return_policy || ""} onChange={(e) => setStoreConfig({ ...storeConfig, return_policy: e.target.value })} placeholder="e.g. We accept returns within 14 days of purchase..." rows={4} style={{ ...inputStyle, fontSize: 12, resize: "vertical" as const }} /></div>
              </div>
            </div>
            <div style={sectionCard}>
