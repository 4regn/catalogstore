...
export async function sendPartnerApprovalEmail(params: {
  ...
  // The bundled UNIK wordmark is white text on a transparent background
  // (built for the site's own dark UI) -- invisible on a plain white email
  // body, so it gets a dark band of its own here rather than sitting
  // directly on white. seller.logo_url isn't used for this: it's whatever
  // colour a seller uploaded for their storefront, which could just as
  // easily be a dark logo that would go invisible on a dark band instead --
  // this file is UNIK-specific already (the Partner program only exists
  // for this template), so referencing UNIK's own known-good asset
  // directly is more reliable than guessing at an arbitrary upload's
  // colour scheme.
  const logoUrl = canonicalStoreUrl(seller.subdomain, "/private-templates/unik-labs/assets/unik-logo-v3-header.png");
  await sendEmail({
    to: partner.email,
    ...
    html: `<div style="max-width:520px;margin:0 auto;font-family:-apple-system,sans-serif">
      <div style="background:#050505;padding:22px 28px;text-align:center;border-radius:16px 16px 0 0">
        <img src="${logoUrl}" alt="${seller.store_name}" style="height:28px" />
      </div>
      <div style="background:#fff;padding:28px;border-radius:0 0 16px 16px;color:#111">
        ...
        <p style="margin:22px 0 0;font-size:12px;color:#888;line-height:1.6">Need a hand with any of this? Live support is built right into your dashboard — just tap Support.</p>
      </div>
    </div>`,
  });
}
