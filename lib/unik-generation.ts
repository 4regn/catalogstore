...
// range. GIANT_FACE, TOON_DRIP and PAPER_CUT each build one portrait from
// a single reference; CHROME_COLLAGE and I_LOVE_MY are fixed five-photo
// collages with a specific slot for each photo.
...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    PAPER_CUT: `DESIGN: TWO NON-NEGOTIABLE RULES BEFORE ANYTHING ELSE: (1) Crop directly beneath the jawline. No neck. No shoulders. No body. Nothing below the chin. (2) If the reference photo shows the subject wearing a hat, beanie, cap or any other headwear, that headwear MUST appear in the final artwork exactly as photographed -- it is never removed, never omitted, never treated as "clothing to exclude". Only clothing on the body below the head is excluded. With those two rules locked in: editorial paper-cut portrait of the head only, directly on ${background}, no card, no rectangular frame or border anywhere -- the collage feathers straight into ${background} with no hard crop line. Extract 2-5 selected facial features as realistic paper cut-outs -- prioritise the eyes -- and reposition them asymmetrically as a handmade mixed-media collage; never warp the face itself, only reposition the cut-outs, each physically glued on with a soft shadow, a torn organic edge and a slight 3-12 degree rotation -- never a smooth digital cutout. Leave at least 60% of the original face untouched, and preserve realistic skin texture and natural lighting throughout -- never apply a black-and-white, greyscale, duotone or heavy colour-grade filter. Optionally enlarge one eye by 150-200%, or duplicate one eye into a second circular or organic cut-out. Add minimal hand-drawn neon-lime marker doodles around the manipulated features only -- circles, highlight marks, radiating lines, scribbles -- kept subtle. Add one small graffiti-style handwritten annotation in solid ${signatureColor} reading \"${line}\", integrated naturally into the collage, plus small restrained typography elsewhere that may read \"${name}\". Neon-lime is the only accent colour. Reminder: head only, no neck, headwear kept exactly as photographed -- gallery-quality, premium and never digitally distorted.`,
