...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    CHROME_COLLAGE: `DESIGN: Premium early-2000s bootleg photo collage on ${background}, dramatic and layered like a polished concert poster. The first photo is the large central hero, cropped tight to the shoulders and tilted very slightly for dynamic energy, with a cinematic rim light and a soft glowing electric-blue edge around the cutout -- no hard frame, border or outline around it, just the glow. The supporting photos overlap around it, also cropped tight to the shoulders as clean cutouts with that same soft glowing electric-blue edge, except for the very last one in a bottom corner, which sits inside one circular chrome frame with bright rim lighting -- that is the ONLY photo with an actual frame around it, and there must be no frame or border of any kind around the outer edges of the whole design. Thread fine electric-blue lightning bolts, light scratches, a subtle film-grain overlay and a few metallic starburst accents behind the cutouts, never over a face. Give each layer a soft drop shadow for a layered, dimensional feel. Typography: massive bevelled 3D chrome Roman-serif caps \"${topText}\" with realistic mirror highlights and a slight upward arch${bottomText ? `, with a second line \"${bottomText}\" directly below it rendered in the exact same large chrome style, weight and scale as the top line -- it must read as equally big, not smaller` : ""}. Silver and electric-blue metallic accents only, fading cleanly into ${background} at the outer edges.`,
    I_LOVE_MY: `DESIGN: Premium early-2000s bootleg romance photo collage on ${background}, glossy and dramatic. The first photo is the large central hero, cropped tight to the shoulders with warm cinematic lighting, set against a soft hot-pink radial glow -- no outline, sticker edge or border around the cutout, just the glow. The supporting photos overlap around it, also cropped tight to the shoulders, each inside its own glossy chrome heart frame with bright rim lighting. Scatter fine hot-pink lightning bolts, soft pink bokeh circles, 6-8 tiny glossy 3D pink decorative hearts, a faint pink halftone-dot texture through the negative space and 8-10 small sparkle accents around the frames, never over a face, plus a subtle film-grain overlay for texture. Top typography: bold puffy 3D bubble-style rounded caps \"I LOVE\" in a pink-to-white gradient with a gentle upward arch. Bottom: small flowing cursive \"My\" above large bold puffy rounded \"${name}\" in the same pink-to-white gradient, both with a soft drop shadow, sitting in front of one subtle horizontal chrome oval accent behind the lockup. Hot-pink and chrome accents only, fading cleanly into ${background} at the outer edges.`,
  };
