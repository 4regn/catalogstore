...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    TOON_DRIP: `DESIGN: Clean bootleg anime merch graphic on ${background}. Reimagine the single reference photo as a premium chibi anime portrait -- oversized head, small narrow shoulders, glossy cel-shaded eyes, clean flat colour fills, confident outlines. Keep the person's real face shape, skin tone, hairstyle and expression clearly recognisable through the stylisation. Small hand-painted brush-scrawl signature near the lower shoulder: \"${name}\"${tagline ? ` with tiny \"${tagline}\" beneath` : ""}. One or two accent colours only.`,
    CHROME_COLLAGE: `DESIGN: Premium early-2000s bootleg photo collage on ${background}. Layer all photos into a bold hero-led collage: the first photo as the large central hero, the rest as smaller supporting portraits layered around it inside circular chrome frames. Add fine lightning, scratches and metallic starburst accents. Top typography: massive chrome Roman-serif caps \"${topText}\"${bottomText ? `. Bottom: chrome Roman-serif \"${bottomText}\"` : ""}. Use silver and electric-blue metallic accents only.`,
    I_LOVE_MY: `DESIGN: Premium early-2000s bootleg romance photo collage on ${background}. Layer all photos into a bold hero-led collage: the first photo as the large central hero, the rest as smaller supporting portraits layered around it inside glossy chrome heart frames. Add hot-pink lightning, sparkles and a radial glow. Top typography: bold rounded caps \"I LOVE\". Bottom: small cursive \"My\" above large bold rounded \"${name}\". Use hot-pink and chrome accents only.`,
  };
