...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const photoOrderRule = count > 1 ? `- The ${count} photos are numbered PHOTO 1 to PHOTO ${count} in upload order; PHOTO 1 is the hero photo. Use each exact photo only in the slot the design below assigns it -- never invent a body part, pose or extra person a photo doesn't show.\n` : "";
  const photoRules = `PHOTO RULES:\n- ${count} reference photo(s) attached. Use EACH exactly once, except stylistic close crops may come from the same visible portrait.\n- Use ONLY the people in the uploaded photos. Never substitute another face.\n- Preserve identity, facial features, skin tone and expression.\n${photoOrderRule}- Do not infer private details or location.\n- ${subjectRule}\n`;
  ...
  const styles: Record<string, string> = {
    ...
    TOON_DRIP: `DESIGN: Clean bootleg anime merch graphic on ${background}. Reimagine the single reference photo as a premium chibi anime portrait, head-and-shoulders only, occupying roughly 75-85% of the canvas height with no full body -- oversized head, small narrow shoulders, glossy cel-shaded eyes that keep the person's real eye shape and spacing, angular eyebrows based on their real eyebrows, clean flat colour fills with hard-edged two-step gradient shadows, crisp variable-weight ink linework, and hairstyle rendered in stylised anime shapes while keeping their real hairline, length and silhouette. Keep the person's real face shape, skin tone and expression clearly recognisable through the stylisation, posed at a confident three-quarter angle. Add a couple of thick dripping paint-splash accents trailing from the top corners, one or two manga speed-lines behind the figure for movement, a light halftone-dot texture through the shadow areas for authentic screen-print merch feel, and a soft drop shadow beneath the figure for depth. Let the lower edge fade softly into ${background} instead of ending in a hard crop. Small hand-painted brush-scrawl signature near the lower shoulder: \"${name}\"${tagline ? ` with tiny \"${tagline}\" beneath` : ""}. One or two saturated accent colours only, picked to complement the reference photo.`,
    CHROME_COLLAGE: `DESIGN: Premium early-2000s bootleg hip-hop photo collage on ${background}, dramatic, layered and polished like a concert poster. Build one dense vertical collage giving every uploaded photo its own fixed role: PHOTO 1 is the large, sharp central hero placed close to the visual centre; PHOTO 2 sits behind it in the upper-left; PHOTO 3 sits behind it in the upper-right, clearly distinct from PHOTO 2; PHOTO 4 sits in the lower-left foreground; PHOTO 5 sits inside its own circular or slightly oval chrome-rimmed frame in the lower-right. Keep every face fully visible. Add one large layered circular frame behind PHOTO 1, a smaller frame around PHOTO 5, fine electric-blue lightning bolts, light scratches, a few metallic starburst accents and a subtle film-grain overlay -- all kept behind faces and away from eyes, nose and mouth -- plus a soft drop shadow under each layer for a dimensional, collaged feel. Top typography: massive bevelled 3D chrome Roman-serif caps \"${topText}\" with realistic mirror highlights and a slight upward arch, sitting behind PHOTO 1's head${bottomText ? `. Bottom: matching chrome Roman-serif \"${bottomText}\" across the lower third` : ""}. Silver and electric-blue metallic accents only, fading cleanly into ${background} at the outer edges.`,
    I_LOVE_MY: `DESIGN: Premium early-2000s bootleg romance photo collage on ${background}, glossy and dramatic. Build one dense vertical collage giving every uploaded photo its own fixed role: PHOTO 1 is the large central hero, positioned slightly above centre with a soft hot-pink radial glow and a glossy sticker-style white outline behind it; PHOTO 2 sits inside its own glossy chrome heart frame in the top-left; PHOTO 3 in a matching heart frame top-right, clearly distinct from PHOTO 2; PHOTO 4 in a heart frame mid-left or lower-left; PHOTO 5 in a heart frame lower-right. Keep every face fully visible. Add fine hot-pink lightning bolts through the middle of the composition, 6-8 small glossy 3D pink decorative hearts, 8-10 small star sparkles and a subtle film-grain overlay -- all kept away from faces and typography -- plus a soft drop shadow under each layer for depth. Top typography: bold rounded caps \"I LOVE\" with a gentle upward arch above PHOTO 1. Bottom: small flowing cursive \"My\" above large bold rounded \"${name}\", both with a soft drop shadow, centred across the lower third. Hot-pink and chrome accents only, fading cleanly into ${background} at the outer edges.`,
  };
