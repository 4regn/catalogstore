...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    TOON_DRIP: `DESIGN: Clean bootleg anime merch graphic on ${background}. Reimagine the single reference photo as a premium chibi anime portrait -- oversized head, small narrow shoulders, glossy cel-shaded eyes, clean flat colour fills, confident bold ink linework with visible brush texture, and subtle two-tone highlight streaks in the hair. Keep the person's real face shape, skin tone, hairstyle and expression clearly recognisable through the stylisation. Add a couple of dynamic paint-drip splashes and one or two manga speed-lines behind the figure for movement, plus a soft drop shadow beneath the figure for depth. Let the lower edge fade softly into ${background} instead of ending in a hard crop. Small hand-painted brush-scrawl signature near the lower shoulder: \"${name}\"${tagline ? ` with tiny \"${tagline}\" beneath` : ""}. One or two saturated accent colours only, picked to complement the reference photo.`,
    CHROME_COLLAGE: `DESIGN: Premium early-2000s bootleg photo collage on ${background}, dramatic and layered like a polished concert poster. The first photo is the large central hero; the rest overlap around it as smaller supporting portraits, alternating between circular chrome frames with bright rim lighting and angled polaroid-style corner frames for visual variety. Thread fine electric-blue lightning bolts, light scratches, a subtle film-grain overlay and a few metallic starburst accents behind the frames, never over a face. Give each frame a soft drop shadow for a layered, dimensional feel. Top typography: massive bevelled 3D chrome Roman-serif caps \"${topText}\" with realistic mirror highlights and a slight upward arch${bottomText ? `. Bottom: chrome Roman-serif \"${bottomText}\"` : ""}. Silver and electric-blue metallic accents only, fading cleanly into ${background} at the outer edges.`,
    I_LOVE_MY: `DESIGN: Premium early-2000s bootleg romance photo collage on ${background}, glossy and dramatic. The first photo is the large central hero with a soft hot-pink radial glow and a glossy sticker-style white outline behind it; the rest overlap around it as smaller supporting portraits, each inside its own glossy chrome heart frame with bright rim lighting. Scatter fine hot-pink lightning bolts, soft pink bokeh circles and 8-10 small sparkle accents around the frames, never over a face, plus a subtle film-grain overlay for texture. Top typography: bold rounded caps \"I LOVE\" with a gentle upward arch. Bottom: small flowing cursive \"My\" above large bold rounded \"${name}\", both with a soft drop shadow. Hot-pink and chrome accents only, fading cleanly into ${background} at the outer edges.`,
  };
