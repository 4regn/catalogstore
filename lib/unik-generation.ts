...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    PAPER_CUT: `DESIGN: Editorial paper-cut portrait built from the head only, directly on ${background} -- crop immediately beneath the jaw, no neck, no shoulders, no body, no clothing below the head, no card, no rectangular frame or border anywhere; the collage feathers straight into ${background} with no hard crop line. Keep any hat, beanie, cap or other headwear the subject is actually wearing exactly as photographed -- never remove or omit it. Extract 2-5 selected facial features as realistic paper cut-outs -- prioritise the eyes -- and reposition them asymmetrically as a handmade mixed-media collage; never warp the face itself, only reposition the cut-outs, each physically glued on with a soft shadow, a torn organic edge and a slight 3-12 degree rotation -- never a smooth digital cutout. Leave at least 60% of the original face untouched, and preserve realistic skin texture and natural lighting throughout -- never apply a black-and-white, greyscale, duotone or heavy colour-grade filter. Optionally enlarge one eye by 150-200%, or duplicate one eye into a second circular or organic cut-out. Add minimal hand-drawn neon-lime marker doodles around the manipulated features only -- circles, highlight marks, radiating lines, scribbles -- kept subtle. Add one small graffiti-style handwritten annotation in solid ${signatureColor} reading \"${line}\", integrated naturally into the collage, plus small restrained typography elsewhere that may read \"${name}\". Neon-lime is the only accent colour. Head only, no neck, cropped directly beneath the jaw -- gallery-quality, premium and never digitally distorted.`,
