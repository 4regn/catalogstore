...
export function buildUnikPrompt(input: UnikGenerationInput) {
  ...
  const styles: Record<string, string> = {
    ...
    PAPER_CUT: `DESIGN: Bold hand-annotated portrait graphic directly on ${background} -- no paper-cutout shapes, no card, no rectangular frame or border anywhere; the portrait itself is the only shape on the design, its edges feathering straight into ${background} with no hard crop line. Keep the portrait fully photographic and full-colour, preserving real skin tones exactly as photographed -- never apply a black-and-white, greyscale or duotone filter. Loosely circle one or two facial details (an eye, the mouth) in thin hand-drawn acid-lime or white line art, each with a small arrow and a couple of short tick marks beside it, plus a few hand-drawn exclamation marks near the hairline. Add one short handwritten annotation reading \"${line}\" near a lower corner, in the same line-art style. Small restrained typography elsewhere may read \"${name}\". Acid-lime is the only accent colour. Keep the composition artful, premium and never distorted.`,
